...
/* `dark` (achado no diff literal desta rodada, 11/09/2026 — Projeto Modelo hoje
   ...
   escuro do N0 (antes só existia a clara, do N1) — ver uso em `PerfisAcesso.tsx`. */
export function Segmented<T extends string>({ options, value, onChange, dark }: { options: { value: T; label: ReactNode }[]; value: T; onChange: (v: T) => void; dark?: boolean }) { return <div style={{ display: 'flex', background: dark ? 'rgba(255,255,255,0.08)' : SOFT, borderRadius: 10, padding: 3, flexWrap: 'wrap', gap: 3 }}>{options.map(opt => <button key={opt.value} onClick={() => onChange(opt.value)} style={{ flex: '1 1 auto', padding: '9px 8px', borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 12.5, background: value === opt.value ? (dark ? DEV_CARD : BRANCO) : 'transparent', color: value === opt.value ? (dark ? '#fff' : INK) : (dark ? '#9B96A8' : TXT3), boxShadow: value === opt.value ? '0 1px 4px rgba(0,0,0,0.12)' : 'none', whiteSpace: 'nowrap' }}>{opt.label}</button>)}</div> }

/* ---- Kit L978: Toggle ---- */
export function Toggle({ value, onChange, label, dark }: { value: boolean; onChange: (v: boolean) => void; label?: ReactNode; dark?: boolean }) { return <button onClick={() => onChange(!value)} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}><div style={{ width: 40, height: 23, borderRadius: 999, background: value ? GREEN : TOGGLE_OFF, position: 'relative' }}><div style={{ width: 18, height: 18, borderRadius: 999, background: BRANCO, position: 'absolute', top: 2.5, left: value ? 19 : 3, transition: 'all .15s', boxShadow: '0 1px 3px rgba(0,0,0,0.3)' }} /></div>{label && <span style={{ fontSize: 13.5, fontWeight: 600, color: dark ? '#fff' : INK }}>{label}</span>}</button> }
/* ---- Kit L980: EmptyState ---- */
