...
export function totalDoLugar(
  ...
  if (conta?.tipo === 'cartao') {
    ...
    return { rotulo: 'Total da fatura', valor: f.total, doMes: f.total }
  }
  const fim = fimDoMes(mesISO)
  const inicio = `${mesISO}-01`
  const anterior =
    (conta?.saldoInicial ?? 0) +
    lancamentos.filter((l) => l.dataCompetencia < inicio).reduce((s, l) => s + l.valor, 0)
  const doMes = lancamentos
    .filter((l) => l.dataCompetencia >= inicio && l.dataCompetencia <= fim)
    .reduce((s, l) => s + l.valor, 0)
  return { rotulo: 'Total acumulado', valor: anterior + doMes, saldoAnterior: anterior, doMes }
}
